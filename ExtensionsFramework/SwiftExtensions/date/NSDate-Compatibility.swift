...
public extension NSDate {
  ...
  @objc static func parseFullISO8601String(_ isoStringDate: Any?) -> NSDate? {
    return Date.parseISO8601String(isoStringDate) as NSDate?
  }
  
  @objc func toString(template templateFormat: String) -> String? {
    let swiftDate = self as Date
    return swiftDate.toString(template: templateFormat)
  }
  
  @objc func isBetweeen(date date1: NSDate, andDate date2: NSDate) -> Bool {
